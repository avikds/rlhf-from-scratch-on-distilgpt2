...
def reward_train_step(model, reward_head, batch, optimizer):
    ...
    # Use positional arguments because the project's _HiddenBackbone
    # does not accept input_ids= or attention_mask= keyword arguments.
    chosen_output = model(
    ...
    # Support both:
    # 1. a raw hidden-state tensor
    # 2. a Hugging Face-style output with .last_hidden_state
    chosen_hidden = (
        chosen_output
        if torch.is_tensor(chosen_output)
        else chosen_output.last_hidden_state
    )

    rejected_hidden = (
        rejected_output
        if torch.is_tensor(rejected_output)
        else rejected_output.last_hidden_state
    )

    # Last non-padding token position for every example.
    chosen_last_idx = (
        batch["chosen_attention_mask"].sum(dim=1).long() - 1
    )

    rejected_last_idx = (
        batch["rejected_attention_mask"].sum(dim=1).long() - 1
    )
    ...
    # Accuracy before the parameter update.
...
# Step 65 - chat
def chat(model, tokenizer, user_message, system_prompt=None,
         max_new_tokens=32, stop_tokens=None):

    tokenizer = set_pad_token_to_eos(tokenizer)

    # Required edge case: don't call the generator with 0 tokens.
    if max_new_tokens <= 0:
        return ""
    ...
    # If the helper returns prompt + generated text, keep only the continuation.
